...
- **Сброс каркаса** — вернуть размер рамки в `frame(...)` к дефолтному
  (`w=530 d=330 h=350`), компоненты и остальные строки не трогаются.
...
### Объявление рамки `frame(w=… d=… h=… [b=…])`
...
набор нижних балок. Параметр `l=` (уровни из старых версий) больше не поддерживается:
полоса на одной высоте собирается из отдельных промежуточных балок.
